...
### 3. MesaNet：最优 Fast Weight Programming
...
**优势**：
- 每个时刻都是最优解（RMS 类比 LMS）
- 计算成本动态分配
- 数值稳定、可 chunk 并行

### 4. LaCT：Large Chunk TTT
...
out_ttt = ttt(x)
out_delta = delta(x)
print(f"TTT output shape: {out_ttt.shape}")
print(f"DeltaNet output shape: {out_delta.shape}")
```
...
def test_memory(model, seq_len):
    ...
    x[:, :seq_len//2, 0] = 1.0  # 关键信息
    with torch.no_grad():
        out = model(x)
    # 检查模型能否在末尾 recall 起始信息
    return (out[:, -1, 0] > 0.5).float().mean().item()
...
def ttrl_step(model, prompt, reward_fn, lr=1e-3):
    """用 REINFORCE 做一步 test-time RL"""
    # 采样
    outputs = model.generate(prompt, num_return_sequences=k)
    rewards = [reward_fn(o) for o in outputs]
    
    # 策略梯度
    log_probs = model.log_prob(outputs)
    loss = -torch.sum(torch.tensor(rewards) * log_probs)
    ...
    return outputs[np.argmax(rewards)]
```
...
**核心参考**：
- [Learning to (Learn at Test Time) - Sun et al. 2024](https://arxiv.org/abs/2407.04620)
- [Test-Time Training on Nearest Neighbors - Hardt & Sun 2024](https://arxiv.org/abs/2305.18466)
- [Titans: Learning to Memorize at Test Time - Behrouz et al. 2024](https://arxiv.org/abs/2501.00663)
- [Test-Time Regression: A Unifying Framework - Wang et al. 2025](https://arxiv.org/abs/2501.12352)
