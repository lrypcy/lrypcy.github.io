...
外循环的目标是学习 slow weights $\theta$，使得内循环的 TTT 能更有效。具体来说：

$$
\theta^* = \arg\min_\theta \sum_{\text{sequences}} \mathcal{L}_{\text{ntp}}(f_\theta(x_1, \dots, x_T))
$$

其中 $\mathcal{L}_{\text{ntp}}$ 是 next-token prediction loss，$f_\theta$ 包含 TTT 层。
...
**为什么更快？** 双偶形式将顺序的外积运算转化为矩阵乘法，更好地利用 GPU/TPU 的并行计算单元。

### TTT-MLP 的对偶形式

对于非线性 fast weights，对偶形式更复杂但原理相同：

$$
Z1_{\text{bar}} = XQ \cdot W1_0 - (\eta \cdot \text{Attn1}) \cdot \nabla_{Z1} + b1_{\text{bar}}
$$

其中 $\text{Attn1} = \text{tril}(XQ \cdot X1^\top)$ 是累积注意力矩阵。
...
**结论**：
- TTT-Linear 在 8k 时已经比 Transformer 快，与 Mamba 持平
- TTT-MLP 因为 memory I/O 较慢，但差距在缩小
- 随着上下文变长，TTT 的优势越来越明显
...
WX = W @ X  # (B, nh, d, K)
delta = (WX - X) @ torch.triu(torch.ones(K, K)).to(X.device) @ X.transpose(-1, -2) @ X
...
**下一篇**：[02-TTT 用于 LLM 推理与工程优化](/2026/09/01/ttt-02-llm-inference/) — TTT-NearestNeighbors、TTT-FewShot、LoRA-TTT、chunk-wise 更新策略。
