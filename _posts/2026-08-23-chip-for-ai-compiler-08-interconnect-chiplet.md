...
延迟公式里的 $$H$$ 是主角。在 $$k \times k$$ 的 mesh 上取两个均匀随机节点，一维上的期望距离是经典等差求和（$$P(\vert\Delta x\vert=d)=2(k-d)/k^2$$）：

$$\mathbb{E}\vert\Delta x\vert = \frac{2}{k^2}\sum_{d=1}^{k-1} d(k-d) = \frac{k^2-1}{3k} \approx \frac{k}{3} \quad\Rightarrow\quad \mathbb{E}H = \frac{2(k^2-1)}{3k} \approx \frac{2k}{3}$$
