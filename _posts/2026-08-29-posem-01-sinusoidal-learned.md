...
**$$m$$ 和 $$n$$ 只以 $$m-n$$ 出现。** 这是一个绝对位置编码偷偷携带相对位置信息的第一个证据。此外，$$\sum_i \cos(\delta\theta_i)$$ 在 $$\delta=0$$ 时取最大值 $$d/2$$；$$\vert\delta\vert$$ 增大时，不同频率的余弦相位逐渐错开、正负相消，内积整体呈衰减震荡——"距离越远，位置相似度越低"，正是语言归纳偏置想要的形状。
...
**Lab 练习**：
1. 用上面的代码画出 $$\boldsymbol{p}_m\cdot\boldsymbol{p}_n$$ 随 $$\vert m-n\vert$$ 的曲线（$$d=128$$，base=10000），观察衰减震荡——预期在 $$\vert m-n\vert\approx 50$$ 时内积降至峰值的 10% 以下，$$\vert m-n\vert\approx 200$$ 时趋近于零；再换 base=100、base=1000000 对比衰减速度，直观感受"底数决定频率谱覆盖"；
