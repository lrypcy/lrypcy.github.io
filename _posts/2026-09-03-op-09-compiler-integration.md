---
title: "算子开发与优化（09）：编译器集成，手写 Kernel 与 torch.compile / MLIR 协同"
...
> **算子开发与优化系列 · 第 09 篇 / 共 13 篇**
>
> [08 国产 NPU](/2026/09/03/op-08-domestic-npu/) ← **本篇** → [10 专家之路](/2026/09/03/op-10-expert-level/)
...
**对算子工程师的启示**：优化搜索的亲历者视角——本系列 10 篇讲的 autotune 搜索空间裁剪（05/10 篇），在大厂编译器里就是用一个 GCN/RL 网络做的。**"性能模型 + 搜索"是无论手写还是编译器都必须面对的核心工具**。
...
**把第 4 节与附录串起来**：自定义算子（手工）→ torch.compile（自动融合）→ MLIR（自研编译器）是一条"控制粒度递增"的谱系；地平线的实践证明了这条谱系在真实 NPU 上的必要性（后端无法复用 → 必须自研编译器），而其 RL/GCN 优化则印证了 10 篇将详述的"AI 驱动的算子/编译器开发"趋势。
...
*上一篇：[08 国产 NPU](/2026/09/03/op-08-domestic-npu/)*
*下一篇：[10 专家之路](/2026/09/03/op-10-expert-level/) —— 算子库设计、性能模型与 AI 驱动开发。*