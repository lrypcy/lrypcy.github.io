...
> * **token 级重要性比在长序列上是病态的**：假设每 token 的 $\log$ ratio 是 $\mathcal N(0.01,0.05^2)$，序列长度 1024 时序列级 ratio 的中位数是 $2.8\times10^4$，4096 时是 $6\times10^{17}$。这就是为什么 GSPO 要把 ratio 换成序列级的几何平均 $\big(\pi_\theta(o\mid q)/\pi_{\rm old}(o\mid q)\big)^{1/|o|}$。
>
...
$$\log\pi_\theta(o\mid q)=\sum_{t=1}^{|o|}\log\pi_\theta(y_t\mid q,y_{<t})$$
$$\mathcal H_t=-\sum_{v}p_{t,v}\log p_{t,v},\qquad \mathrm{PPL}(o)=\exp\Big(-\frac{1}{|o|}\log\pi_\theta(o\mid q)\Big)$$
...
RL 里加的 entropy bonus 通常是 token 级平均 $\frac{1}{\sum|o_i|}\sum_{i,t}\mathcal H_{i,t}$。注意：**熵奖励会让分布变平**，与 KL 惩罚（拉回 $\pi_{\rm ref}$）方向相反，两者系数要一起调。
...
$$\rho^{\rm GSPO}(o)=\Big(\frac{\pi_\theta(o\mid q)}{\pi_{\rm old}(o\mid q)}\Big)^{1/|o|}=\exp\Big(\frac{1}{|o|}\sum_{t=1}^{|o|}\log\frac{\pi_\theta(y_t\mid q,y_{<t})}{\pi_{\rm old}(y_t\mid q,y_{<t})}\Big)$$
