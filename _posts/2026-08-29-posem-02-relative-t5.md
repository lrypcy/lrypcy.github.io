...
> * **为什么它们最终都让位于 RoPE**：① 改写了注意力的矩阵形式，与 FlashAttention/张量并行/KV cache 等工程优化摩擦大；② T5 的 bucket 截断使相对距离上限=外推上限；③ 加性 bias 无法随 $|m-n|$ 连续衰减成可调的注意力局部性。RoPE 用"不改公式、只旋转 Q/K"一次解决了全部三点。
...
**Lab 练习**：
1. 在一个 2 层小 Transformer 上分别接 T5 bias 与不加位置编码，训练一个"复制间隔 k 的 token"的合成任务（如 `a b c ...`，输出与首 token 相距 10 的那个字符），对比两种设置对精确距离的敏感度；
2. 打印训练后 T5 bias 各头的桶偏置曲线 $b_c$，观察哪些头学到了单调递减（局部性）、哪些头学到远程偏好——验证"逐头距离偏好"的说法。
