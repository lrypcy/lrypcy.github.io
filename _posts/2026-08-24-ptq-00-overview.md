...
* **反直觉发现**：(1) 教科书公式 $$s = \max\vert x\vert/2^{b-1}$$ 只在**均匀分布 + 高位宽**下接近最优，对 4-bit 高斯分布它比最优 scale 大 ~50%，白扔约 3 dB 精度（详见 §1.3 与附录 Demo 2）；(2) 量化误差的主力往往不是舍入（rounding）而是**裁剪（clipping）**——GPTQ 一类方法修的是前者，AWQ 修的是后者；(3) per-channel 粒度对“输出通道范数差异”有效，但对**输入通道离群**无效——这才是 group 量化成为主流的真正原因（§3.3）。
...
1. **舍入误差（granularity error）**：$$\vert x\vert$$ 落在网格内时，$$\epsilon \sim U(-\tfrac{s}{2}, \tfrac{s}{2})$$，方差 $$s^2/12$$；
2. **裁剪误差（clipping error）**：$$\vert x\vert$$ 超出 $$[z + q_{\min}s,\ z + q_{\max}s]$$ 时被截断。
...
$$
s = \frac{\max(\vert\min x\vert,\ \vert\max x\vert)}{2^{b-1} - 1}
$$
...
- **舍入项**：$$\vert x\vert \le M$$ 的概率为 $$t$$，条件方差 $$s^2/12$$，贡献 $$t \cdot s^2/12$$；
- **裁剪项**：$$\vert x\vert > M$$ 的概率为 $$1-t$$，对 $$x \in (M, a]$$，$$E[(x-M)^2] = (a-M)^2/3$$，贡献 $$(1-t)(a-M)^2/3$$。
...
对比教科书公式：$$2a$$ 是分布全宽，所以 $$s^\ast = \text{range}/(2^b - 1)$$，而 folklore 公式 $$s = \max\vert x\vert/2^{b-1} = \text{range}/2^b$$。**两者在 8-bit 时只差 $$256/255 \approx 0.4\%$$，在 4-bit 时差 $$16/15 \approx 6.7\%$$**。注意 folklore 公式并非“完全不裁剪”——它对应的覆盖率 $$t = (2^{b-1}-1)/2^{b-1}$$ 略小于 1，是“轻微裁剪换舍入精度”的隐式权衡；而均匀分布下真正的闭式最优是 $$2a/(2^b-1)$$。这就是“$$s \approx \max\vert x\vert/2^{b-1}$$ 对均匀分布近似最优”这句话的精确含义：**近似误差随位宽下降，4-bit 以下开始不可忽略**。
...
| 位宽 $$b$$ | 最优裁剪点 $$u^\ast = M^\ast/\sigma$$ | 最优步长 $$s^\ast = u^\ast\sigma/q_{\max}$$ | folklore $$s = \max\vert x\vert/2^{b-1}$$（$$n=2\times10^5$$ 时 $$\max\vert x\vert\approx 4.5\sigma$$） |
|---|---|---|---|
| 4 | $$\approx 2.5$$ | $$\approx 0.36\sigma$$ | $$\approx 0.56\sigma$$（**偏大 56%**） |
| 8 | $$\approx 4.0$$ | $$\approx 0.031\sigma$$ | $$\approx 0.035\sigma$$（偏大 13%） |

**结论**：位宽越低，基于经验 $$\max\vert x\vert$$ 的 folklore 公式越差——4-bit 时它把步长放大 56%，对应 SNR 损失约 3 dB（附录 Demo 2 实测）。原因：经验 max 是样本量的函数（$$n=10^5$$ 时 $$\max\vert x\vert \approx 4.4\sigma$$，$$n=10^6$$ 时 $$\approx 4.75\sigma$$），而最优裁剪点只由分布决定。这直接解释了：
...
校准的本质是估计激活分布的各阶统计量：scale 需要 $$\max\vert x\vert$$ 或 $$\sigma$$（§1.3），AWQ 需要 $$E[\vert x_k\vert]$$（通道重要性），GPTQ 需要 $$H = E[xx^\top]$$（见 4.3）。
...
做法：对激活建直方图，对每个候选阈值 $$T$$ 把 $$\vert x\vert > T$$ 的部分折叠进边界 bin，计算量化后分布 $$Q_T$$ 与原始 $$P$$ 的 KL，选最小者。它天然容忍长尾（尾部折叠而非裁剪），但对低位宽不敏感（bin 太粗时 KL 退化为平凡解）。
...
- **SmoothQuant**（arXiv:2211.10438）：观察到 $$H_{kk}$$ 的长尾（少数输入通道激活极大），用迁移因子 $$\alpha$$ 把激活的难量化性**转移**到权重侧：$$\hat{x} = x \cdot \mathrm{diag}(s)^{-1}$$，$$\hat{W} = \mathrm{diag}(s) \cdot W$$，其中 $$s_k = \max\vert x_k\vert^\alpha / \max\vert W_k\vert^{1-\alpha}$$。数学上保持 $$Wx$$ 不变（等价变换），工程上让两侧都落入 8-bit 可表示范围。
...
**② 激活感知类（SmoothQuant / AWQ / OmniQuant）**：共同点是**承认“权重误差要在激活空间里算”**。SmoothQuant 用等价变换把激活离群迁移到权重，达成 W8A8 且无需训练；AWQ 用 $$E[\vert x_k\vert]$$ 识别重要通道、做 0.1% 级别的缩放 + 裁剪，4-bit 精度逼近 GPTQ 但无 Hessian 求逆；OmniQuant 把 SmoothQuant 的迁移因子和 AWQ 式裁剪做成**可学习参数**（轻量 QAT 味道，但只训几十步）。这条路线是 2023-2024 年部署主流。
...
**通用经验法则**：位宽越低，越要“激活感知”（用 $$H$$ 或 $$E[\vert x\vert]$$）；数据越少，越要“解析/旋转”（HQQ/QuaRot）；硬件越老，越要“格式保守”（INT8 而非 FP8）；任务越敏感（代码/数学），越要保留 1 位余量（4-bit 用 group=128 而非 256）。
...
1. **画激活分布**：按通道统计 $$\max\vert x\vert$$ 与直方图，确认离群轴（输入通道 vs 输出通道）与长尾程度——决定粒度与是否需迁移/旋转（§8.1）；
...
- **Demo 2**：naive 的 $$s = \max\vert x\vert/8 \approx 0.55$$，最优 $$s \approx 0.35$$（$$k^\ast \approx 2.5$$），SNR 提升约 3 dB——**4-bit 下经验 max 是糟糕的 scale 估计器**，这正是 HQQ/AWQ 裁剪的动机；
