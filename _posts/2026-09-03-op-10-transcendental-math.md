...
> **有一篇纵深续篇**：[《超越函数在 GPU 上到底怎么算：从 MUFU.EX2 到 FlashAttention-3 的 softmax 之战》](/2026/09/26/gpu-transcendental-softmax-llm/) —— 只盯 NVIDIA 一家，打通「硬件怎么算」与「LLM 为什么撞上这个瓶颈」，并给出可复现的数值实验。本篇的部分数据在那篇里做了更正，建议对照阅读。
...
| **SFU（Special Function Unit）** | **每 SMSP 一个（SM 内共 4 个）** | 超越函数 |
| Tensor Core | 4 | 矩阵乘 |

SFU 的吞吐：**每 SM 每周期 16 次**。相对 FP32 的比例随架构变化——A100 的 FP32 是 64/周期，所以是 **1/4**；H100 的 FP32 翻倍到 128/周期，就变成 **1/8**。
...
**精度代价**：`__expf` 走的是 `MUFU.EX2`，误差约 $$2^{-22} \approx 2.4\times 10^{-7}$$（~2 ULP，见 [PTX ISA](https://docs.nvidia.com/cuda/parallel-thread-execution/)），而 `expf` 保证 < 2 ULP 且在全输入域成立。对 Softmax 而言 `__expf` 完全够用——这也是 cuDNN / FlashAttention 内部敢用快速版的原因。

> **更正（2026-09-26）**：此处原文写「$$10^{-5}\sim 10^{-6}$$ 量级」，偏保守约 100 倍。实测（见[纵深篇](/2026/09/26/gpu-transcendental-softmax-llm/) Lab 4）FP32 softmax 端到端最大绝对误差为 $$1.3\times 10^{-7}$$。
...
- TPU 没有公开的专用超越函数指令文档，但已知其 TensorCore 旁配有独立的**非线性 / 超越函数单元**；
- XLA/HLO 里 `exp`/`log`/`rsqrt` 在 TPU 上主要落在 **Vector 单元**，以软件多项式逼近实现；
- bf16 精度下多项式阶数需求低，软实现成本可接受。

> **更正（2026-09-26）**：原文「TPU 没有专用超越函数单元」措辞过于绝对，已改为上面的保守表述。
...
| `__expf` | ~2e-7（$$2^{-22}$$） | Softmax/Attention/GELU |
...
*纵深篇：[超越函数在 GPU 上到底怎么算：从 MUFU.EX2 到 FlashAttention-3 的 softmax 之战](/2026/09/26/gpu-transcendental-softmax-llm/) —— 本篇的续篇，含数值实验与数据更正。*