...
**最优误差**：把 $$\delta^\top = c H^{-1}_{:,q}/[H^{-1}]_{qq}$$ 代回二次型，利用 $$\bigl(H^{-1}_{:,q}\bigr)^\top H H^{-1}_{:,q} = [H^{-1}]_{qq}$$（因为 $$H H^{-1} = I$$），得到量化第 $$q$$ 个权重后的最小误差增量：
...
**group 与 GPTQ 更新的交互**：scale 必须在量化该 group 的**第一个权重之前**用"当前值"算好。由于 Lazy-Batch 的 batch 大小 $$B = 128$$ 恰好整除 group_size，scale 计算与 batch 边界对齐：量化第 $$q$$ 列时，若 $$q$$ 是 group 起点，用当前 $$w$$ 的 group 段计算 $$\text{scale} = \max\vert w_{seg}\vert/(2^{b-1}-1)$$，然后该组内所有列的量化都引用这个 scale。
...
GPTQ 默认使用**对称量化**：$$q = \operatorname{clip}(\operatorname{round}(w/\text{scale}), -2^{b-1}, 2^{b-1}-1)$$，scale 由 $$\max\vert w\vert$$ 决定，zero-point 恒为 0。对称量化的好处是 kernel 实现简单（只需一次乘加，无需反量化加法），且对权重这种"以 0 为中心"的分布几乎无损。
