---
title: "LLM PTQ 深度解析（02）：GPTQ：基于 Hessian 的二阶权重量化"
date: 2026-08-24 10:20:00 +0800
categories:
  - 模型量化
tags: [quantization, ptq, gptq, hessian]
layout: post
mathjax: true
---

> **系列导航**：本文是《大模型 PTQ 量化》系列的第 2 篇。第 0 篇《量化全景》[量化全景](/2026/08/24/ptq-00-overview/)建立了 RTN / PTQ / QAT、精度-内存权衡等基础概念；第 1 篇《RTN/LLM.int8》[RTN 与 LLM.int8()](/2026/08/24/ptq-01-rtn-llmint8/)介绍了最朴素的舍入量化与 LLM.int8() 的混合精度分解。本文进入全系列数学最深的一篇：GPTQ 用 Hessian 矩阵承载的二阶信息，把"量化误差最小化"变成可精确求解的凸优化问题。后续第 3 篇 AWQ/OmniQuant、第 4 篇 SpQR/OWQ/HQQ 都将以本文的数学框架为参照系展开。
...
在第 1 篇里我们看过 RTN（Round-To-Nearest）的表现：8-bit 时几乎无损，4-bit 时全线崩溃。以 OPT-175B 为例，RTN 在 4-bit 下 WikiText-2 困惑度急剧恶化到 100 以上，而 FP16 基线只有 8.34（来源：arXiv:2210.17323 Figure 2）。为什么？

因为 RTN 对每个权重**独立**舍入，完全不考虑权重之间的耦合关系。一个权重被舍入 0.001，另一个被舍入 0.002，误差可能同向叠加；更关键的是，**权重对模型输出的影响不是均匀的**——某些权重哪怕只动 0.001，也会让激活产生巨大偏移。第 1 篇中 LLM.int8() 的 outlier 分析已经暗示了这一点：极少数"敏感"维度主导着量化误差。
...
于是，问题从"组合优化"（搜索所有量化格点组合，指数爆炸）变成了"**带约束的二次优化**"（每个权重独立落到最近的格点，但允许其他权重连续补偿）。这就是 OBQ 的数学起点，也直接呼应了第 1 篇末尾埋下的伏笔：敏感度分析要从"一阶梯度"升级为"精确的二阶 Hessian"。
...
**为什么量化能"白赚"内存**：权重只读、推理时无需梯度，因此权重精度可以压到极低；而激活必须保持较高精度（这是 SmoothQuant 系列的主题，第 6 篇展开）。GPTQ 瞄准的正是"权重 4-bit、激活 FP16"这一不对称设定。
...
这就是"权重敏感度 = Hessian 对角元"的严格推导：$H_{qq} = 2\sum_t X_{qt}^2$ 正是第 $q$ 个输入维度的能量（激活二阶矩的 2 倍）。第 1 篇里我们凭直觉说"激活幅度大的维度敏感"，这里给出了精确公式。
...
第 3 篇的主角 AWQ（arXiv:2306.00978）在论文里直接与 GPTQ 对比。以 LLaMA-7B、WikiText-2、4-bit 为例（来源：arXiv:2306.00978 Table 2）：
...
看起来单卡能放下，但别忘了**每一层都要存**：逐层前向收集激活时，整个模型的中间激活也要驻留（或重算）。Cholesky 分解本身 $O(d_{col}^3)$：$d_{col} = 16384$ 时约 $4.4 \times 10^{12}$ 次浮点运算/层，数百层叠加就是数 GPU 小时——这正是"175B 要 4 小时"的原因。**内存墙限制了 GPTQ 无法直接用于 $d_{col} > 32K$ 的超宽模型（如某些 MoE 的 expert 层），计算墙则让"量化一次"成为小时级任务**。后续的 QuIP#（第 5 篇）用 Hadamard 旋转把 Hessian"洗"成近似单位阵，本质就是在拆这堵墙。
...
这三条边界共同决定了：**GPTQ 的安全区在 3-4 bit**，2-bit 需要额外技巧（act_order、更小的 group），1-bit 则是 QuIP#/AQLM 的战场（第 5 篇）。
...
|---|---|---|---|
| 激活感知 | AWQ / OmniQuant | 放弃 Hessian，用激活幅值 | 第 3 篇 |
| 异常值保护 | SpQR / OWQ / HQQ | 用 Hessian 找异常值，单独高精度 | 第 4 篇 |
| 非相干处理 | QuIP# / AQLM | 旋转使 Hessian 近似单位阵 | 第 5 篇 |
| 激活平滑 | SmoothQuant / ZeroQuant | 转移量化难度到权重侧 | 第 6 篇 |
...
它的局限也定义了后续十年的研究议程：Hessian 太贵 → AWQ 放弃它、QuIP# 洗白它；校准太脆 → 无校准方法（HQQ、ZeroQuant-FP）出现；逐层太保守 → 端到端可微量化（QAT 与蒸馏）在更小模型上回归。**理解 GPTQ，就理解了 PTQ 这个领域 80% 的坐标系**——这也是本系列把它放在第 2 篇、并且把数学推到最深处的原因。
...
**Q5：量化后还能微调吗？**
可以，但意义不大——GPTQ 的权重已经被"补偿"过，直接微调会破坏补偿结构；若要微调，应基于**反量化后的 FP16 权重**做 LoRA（冻结基座），这是当前的主流做法。真正的"量化感知微调"（QAT）是另一条路线，见系列第 0 篇的 QAT 部分。

**Q6：GPTQ 和 GGUF 的 Q4_K_M 是什么关系？**
GGUF 的 k-quants（第 8 篇）是 llama.cpp 自研的启发式分块格式，**不是** GPTQ；两者都做 4-bit 权重量化，但 k-quants 无需校准数据、精度略低，GPTQ 需要校准但精度更高。转换工具（如 AutoGPTQ → GGUF）只是格式迁移，数学内核不同。
...
7. **系列其他篇目**：[第 1 篇 RTN/LLM.int8](/2026/08/24/ptq-01-rtn-llmint8/)、[第 3 篇 AWQ/OmniQuant](/2026/08/24/ptq-03-awq-omniq/)、[第 5 篇 QuIP#/AQLM](/2026/08/24/ptq-05-quip-aqlm/)。

---

*感谢阅读。本文是《大模型 PTQ 量化》系列第 2 篇，数学密度为全系列之最——如果 Lagrange 乘子与 Sherman-Morrison 的推导让你卡壳，建议对照第 2.2-2.3 节手推一遍，再回头跑一遍第 6 章的代码，会有"原来如此"的感觉。*

我是 **peicy1**，专注于 AI Infra 和大模型部署。下一篇《AWQ/OmniQuant：激活感知的权重量化》将展示"没有 Hessian 的世界"如何用更轻的代价逼近 GPTQ 的精度。
