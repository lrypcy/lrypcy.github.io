...
$$
\epsilon = \epsilon_r + \epsilon_c, \qquad \vert\epsilon_r\vert \le s/2, \qquad \epsilon_c = \begin{cases} x - s\cdot q_{\max} & x > s\cdot q_{\max} \\ x - s\cdot q_{\min} & x < s\cdot q_{\min} \\ 0 & \text{otherwise} \end{cases}
$$
